...
def dis(input_shape):
    ...
    x = conv_block(combined_img, 8, is_first_layer=True)#(<your creativity>)
    x = conv_block(x, 16)#(<your creativity>)
    x = conv_block(x, 32)#(<your creativity>)
    x = conv_block(x, 64)#(<your creativity>)
    # 輸出 通常是1維輸出 不過這裡的Example Code 是Patch GAN的Dis 所以你要直接Dense or Conv2D都是可以的
    # Patch gan是說 只要
    # Select 1 Patch-GAN Dis 
    x = conv_block(x, 1, is_last_layer=True)#(<your creativity>)(x)
    # Select 2 Dense 
    # 不過做Desne前 應該需要Flatten喔 不過我有點懶 就不提供flatten的代碼了
    # x = Dense(32)(x)#(<your creativity>)(x)
...
def gen(input_shape):
    ...
    # Encoder ==> 將圖做降維
    x1 = conv_block(img_A, 4, is_first_layer=True) #降維step1   # output_size (None,filter_A,A,A)
    x2 = conv_block(x1, 8) #降維step2   # output_size (None,filter_B,B,B)  #當然A>B>C>D 
    x3 = conv_block(x2, 16) #降維step3   # output_size (None,filter_C,C,C)  #就是輸出的Size愈來愈小
    x4 = conv_block(x3, 32) #降維step4   # output_size (None,filter_D,D,D)  #然而filter_A<=filter_B<=filter_C<=filter_D
    ...
    # Middle_vector
    x5 = conv_block(x4, 32) #重新提取同維度的特徵  #size (None,filter_D,D,D)
    # Decoder ==> 將降維後的Middle Vector 升維至原圖
    d1 = deconv_block(x5, x4, 32) #升維Step1 # output_size (None,filter_D',D,D)
    d2 = deconv_block(d1, x3, 16) #升維Step2 # output_size (None,filter_C',C,C)
    d3 = deconv_block(d2, x2, 8) #升維Step3 # output_size (None,filter_B',B,B)
    d4 = deconv_block(d3, x1, 4) #升維Step4 # output_size (None,filter_A',A,A) #沒錯就是反操作 假設你conv 是每次經過 降兩倍的size 那deconv就是升兩倍的size
    ...
    out_img = Conv2D(1, name='out_img', kernel_size=(4,4), strides=(1,1), activation='tanh', padding='same')(d5) #這邊記住 是output_img 所以你的fiter要調整成 1因為我們這次的作業是灰階圖 不是彩色的
...
input_shape=(channels,img_row,img_col) #(channels,img_row,img_col) # or (img_row,img_col,channels)
